...
TEST_CASE("input_output_basic","io")
{
  ...
  const auto& f1 = field_repo->get_field("field_1","Physics");
  const auto& f2 = field_repo->get_field("field_2","Physics");
  const auto& f3 = field_repo->get_field("field_3","Physics");
...
std::shared_ptr<FieldRepository<Real>> get_test_repo(const Int num_lcols, const Int num_levs)
{
  ...
  const auto& f1 = repo->get_field(fid1);
  const auto& f2 = repo->get_field(fid2);
  const auto& f3 = repo->get_field(fid3);
  auto f1_hst = f1.get_view<Host>();
  auto f2_hst = f2.get_view<Host>();
  auto f3_hst = f3.get_reshaped_view<Real**,Host>();
  for (int ii=0;ii<num_lcols;++ii) {
