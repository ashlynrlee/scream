...
namespace scream {
  ...
  using MemberType = KT::MemberType;
    ...
    TEST_CASE("rrtmgp_scream_stand_alone", "") {
        ...
        auto sw_flux_up_test = real2d("sw_flux_up_test", d_sw_flux_up.data(), ncol, nlay+1);
        auto sw_flux_dn_test = real2d("sw_flux_dn_test", d_sw_flux_dn.data(), ncol, nlay+1);
        auto sw_flux_dn_dir_test = real2d("sw_flux_dn_dir_test", d_sw_flux_dn_dir.data(), ncol, nlay+1);
        auto lw_flux_up_test = real2d("lw_flux_up_test", d_lw_flux_up.data(), ncol, nlay+1);
        auto lw_flux_dn_test = real2d("lw_flux_dn_test", d_lw_flux_dn.data(), ncol, nlay+1);
        {
          const auto policy = ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncol, nlay);
          Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const MemberType& team) {
            const int i = team.league_rank();
